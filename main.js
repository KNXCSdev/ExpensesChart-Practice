...
async function loadGraphData() {
  try {
    ...
    let maxAmount = Math.max(...arr.map((item) => item.amount));
    ...
    graphElements.forEach((element) => {
      element.classList.remove("main__graph--highest");
    });
    ...
    graphElements.forEach((element, index) => {
      ...
      if (valueElement) {
        ...
        if (arr[index].amount === maxAmount) {
          element.classList.add("main__graph--highest");
        }
      }
    });
  } catch (error) {
